...
import tempfile
import wave
from speechapi import formatConverter
from speechapi import googleApi
from speechapi import baiduApi
...
def main(mp4_file, format='vtt'):
    parser = argparse.ArgumentParser()
    parser.add_argument('source_path', help="Path to the video or audio file to subtitle", nargs='?' ,default=mp4_file)
    parser.add_argument('-C', '--concurrency', help="Number of concurrent API requests to make", type=int, default=10)
    parser.add_argument('-o', '--output',
                        help="Output path for subtitles (by default, subtitles are saved in \
                        the same directory and name as the source path)")
    parser.add_argument('-F', '--format', help="Destination subtitle format", default=format)
    ...
    converter =  formatConverter.FLACConverter(source_path=audio_filename)
    google_recognizer = googleApi.SpeechRecognizer(language=args.src_language, rate=audio_rate, api_key=GOOGLE_SPEECH_API_KEY)
    baidu_recognizer = baiduApi.SpeechRecognizer(language=args.src_language, rate=audio_rate)
    ...
    if regions:
        try:
            ...
            pbar = ProgressBar(widgets=widgets, maxval=len(regions)).start()
            # multiple
            for i, transcript in enumerate(pool.imap(baidu_recognizer, extracted_regions)):
...
if __name__ == '__main__':
    ...
    # kuaiji-09, kuaiji-11
    mp4_file = '/home/zhangcl/temp/mp4/kuaiji-11.mp4'
    format = 'json'
    sys.exit(main(mp4_file, format))
